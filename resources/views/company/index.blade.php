...
@section('content')
    ...
    @else
    <h2 class="mt-3">{{ __('app.companies list')}}</h2>
    <div class="card-columns">
        @foreach($companies as $company)
        <div class="card">
            <a href="{{ route('company.show', ['id' => $company->id]) }}"><img src="/storage/img/company/{{ $company->image }}" alt="{{ $company->{'name_'.app()->getLocale()} }}" class="card-img-top" /></a>
        <div class="card-body">
            <h5 class="card-title"><a href="{{ route('company.show', ['id' => $company->id]) }}">{{ $company->{'name_'.app()->getLocale()} }}</a></h5>
            <p class="card-text">{{ $company->city->{'name_'.app()->getLocale()} }}</p>
            </div>
        </div>
        @endforeach
    </div>
